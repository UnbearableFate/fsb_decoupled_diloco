...
1. per-merge 证据字段落地：每次 merge 记录"加权平均 base 落后量"（Σ staleness×有效权重/Σ 权重）、staleness 直方图、fresh 权重占比（STL-01）；分析脚本可对"merge 的 staleness 构成 vs 随后的 validation/loss 变化"做回归式分析（STL-02）；
2. λ 消融完成：λ ∈ {0.25(基线), 更强降权至少 2 档, fresh-only 对照}，P6 三件套 + validation 指标，结论入 run_analysis（STL-03）;
...
- **非目标**：base-relative displacement 实现（00 §4.5 的设计工作，另立计划）；staleness 上限 `max_staleness_versions` 的语义改动；merge 数学任何变更。
...
| L1 分析能力 | 回归分析的最小输出形态冻结 | 分析脚本加列/加图 | STL-02：对一个既有 run 产出示例分析 |
| L2 消融 | 实验矩阵冻结（λ 档位、seeds、validation 口径）；等待新基线 | 无代码（配置） | 消融 run 组 + STL-03 结论；STL-04 决策成文 |
