...
| 顺序 | 计划 | 一句话目标 | 性质 | 前置 |
| --- | --- | --- | --- | --- |
| 1 | [S4-stop-predicate-unification.md](S4-stop-predicate-unification.md) | 合并 `stop_requested`/`fragment_stop_requested`，同时修复 B1 | **含语义变更** | 无 |
| 2 | [S2-prediction-reconcile-dedup.md](S2-prediction-reconcile-dedup.md) | 消除 prediction reconcile 双拷贝；交付事件轨迹等价工具 | 行为保持 | 无 |
...
顺序依据：S4 最小且修复确认 bug，收益最先兑现；S2 交付所有后续计划共享的等价性基础设施并降低 S1 风险；S5 依赖 S1 的策略类存在。S3 与 S1 互相独立，可并行，但报告与 run 对照应分开。
...
- 除非计划明确标注"语义变更"，一律为**行为保持重构**：以基线 commit 上的 tiny run 事件轨迹为等价基准（工具由 S2 交付）。
