...
- 前置依赖：无。后续 S1 将把本计划的 helper 吸收进 prediction 策略类，接口设计需为此留出余地（纯函数、状态经参数传入传出）。
...
2. 事件轨迹等价工具 `fs_diloco/tools/compare_event_traces.py` 交付并自测通过（REC-01）。
...
- 归一化：从 JSONL 事件流提取有序 `(role, event_name, 稳定字段子集)` 序列。默认丢弃：时间戳、耗时/秒数类字段、速率、主机名、PID、绝对路径、update_id 中的时间成分。保留：事件名、版本号、step、token 计数、reason、布尔标志。字段取舍以每事件白名单 profile 配置，便于按计划调整。
- 输出：等价 → exit 0；不等价 → exit 1 并打印首个分歧位置的两侧上下文。
- 自测：同一轨迹对比等价；注入乱序/字段改动/事件缺失后对比必须报告不等价。
- 局限声明：多进程 tiny run 中 learner 间相对顺序受调度影响，对比按**每 learner 单独序列**进行，不断言跨 learner 交错顺序。

### 4.2 reconcile helper

- 形态：模块级纯函数，prediction 状态（`reference_flat`、`carried_tokens`、`update_id`、`base_version` 四元组）以一个轻量 dataclass 进出，不读写闭包变量——这是 S1 迁移的直接前提。
- 职责边界（保证事件顺序逐字节不变）：
...
| --- | --- | --- | --- |
| L0 轨迹工具 | 写自测：等价样例、乱序反例、字段漂移反例先 RED | 实现 `compare_event_traces.py` 与 profile 机制 | REC-01 GREEN；用一次真实 tiny run 日志作冒烟输入；工具用法写入 progress.md |
| L1 基线冻结 | 记录基线 commit + dirty 状态 | 在基线上跑 tiny prediction run（`configs/fs_diloco_tiny_predict_local.yaml`，固定 seed） | 归一化轨迹与原始日志存入 artifacts，作为 L2 的对照物 |
...
| ID | 测试 | 通过条件 |
| --- | --- | --- |
| REC-01 | 轨迹工具自测 | 等价样例 exit 0；乱序/字段漂移/缺事件反例 exit 1 且指出首个分歧 |
