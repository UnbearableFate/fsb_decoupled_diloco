...
- 来源：review S3（中）。四处结构性重复：
  ...
  4. learner 侧 `run_fragment_learner` 内 fragment adoption 块实际有四个调用语境：inner poll、upload 后、final wait 循环、退出前最终 latest（原 review 把后两者合写为 1332-1381）。
...
- 前置依赖：S2 的轨迹等价工具。与 S1 互相独立；full global strategy 与 fragment adoption 语义不同，S1 是否完成不改变本计划的 helper 边界。
...
- 事件名与字段保持现状（受控测试与已证明可重复的轨迹投影是硬门槛），骨架不得为了统一而重命名事件；
- 若 diff 清单暴露两侧**无意的行为分歧**（同 S1 §10 的处理方式）：先记录，经确认后要么保持分歧（参数化表达出来），要么定义为 bug 另行修复，不得在重构中静默抹平。

### 4.2 input-closed 谓词

- 当前 `all_expected_learners_stopped(store, config)` 已是共享入口，且同时被 shutdown 路径使用；本轮不得为了“看起来有改动”复制或改名。先用组合测试证明其 expected/stopped 集合语义；仅在测试暴露可测性缺口时提取一个接收 learner rows 与 expected IDs 的纯子函数。
- fragment 主循环**不**调用该谓词（本轮），B4 接线应能直接复用现入口。

### 4.3 learner fragment adoption 函数

- 四个调用语境（inner poll / upload 后 / final wait 循环 / 最终 latest）先 diff 盘点差异（事件名、字段、optimizer reset、token 清零、等待语义），收敛为单一函数 + 少量调用点参数；
...
| --- | --- | --- | --- |
| L0 基线冻结 | 记录 commit；差异盘点：两对 syncer 函数与四个 learner adoption 语境的逐行 diff 清单；同代码重复 run 识别异步不可重复事件 | 基线上跑 tiny full（`fs_diloco_tiny_local.yaml`）与 tiny fragment（`fs_diloco_tiny_fragment_local.yaml`）run | 可重复投影、原始轨迹 + diff 清单入 artifacts/progress |
...
| L2 input-closed 谓词 | 谓词单元测试先行：现语义全组合 | 现 helper 已满足则只加测试；仅在需要时提炼纯子函数并保持 full 调用行为 | LDU-07 GREEN；fragment 主循环不接线的事实写入 progress |
| L3 learner adoption 去重 | LDU-03 先 RED：单函数在四种调用语境下事件与状态转换一致于 diff 清单 | 提炼函数，四处调用点改写 | fragment 可重复投影与终态不变量等价（LDU-05）；`tests/test_fragment_pipeline_smoke.py` 通过 |
...
| LDU-03 | fragment adoption 单函数 | 四种调用语境（inner poll/upload 后/final wait 循环/最终 latest）的事件、token 清零与 optimizer reset 语义一致 |
| LDU-04 | full 行为等价 | 受控单元/集成测试全绿；tiny full 的**已证明可重复**核心 publication 投影与终态不变量同基线一致 |
| LDU-05 | fragment 行为等价 | 受控单元/集成测试全绿；tiny fragment 的**已证明可重复**核心 merge/adoption 投影与终态不变量同基线一致 |
