...
```text
PROTOCOL_VERSION                  = 4
AUTHORITY_SCHEMA_VERSION          = 5   # P3 incompatible DDL revision; FullProtocol仍为v4
...
- **INIT-01**：正式 shared FS 已在 P0 证明不支持 directory `RENAME_NOREPLACE`。initializer 必须先把 staging 中的 identity object hard-link create-no-replace到 final同parent的sibling reservation并fsync parent，再exclusive `mkdir(final)`，逐对象按manifest SHA-256 hard-link/fsync，最后发布create-no-replace `.complete` hashed manifest。reservation与run同生命周期、不得由generic cleanup删除；marker前只有持有reservation同一inode的原staging可以恢复，其他同identity staging fail closed；新建reservation后发现final已存在必须删除本次reservation并可重复fail closed。首次发布的strict validator拒绝协议外目录项；actor正常启动只做有界的manifest/identity/immutable-object校验，绝不能递归扫描`audit/**`等mutable subtree。已完成final若reservation丢失，只能在显式、可预期较慢的repair中完整核验in-directory identity、manifest、对象hash和精确条目集合后，从`final/.identity`以same-inode hard-link修复reservation；普通publish/retry不能借此认领既有final。
- **ENV-01**：descriptor 冻结 source/lock/model/tokenizer/dataset revision；P3冻结actor runtime attestation schema/API，P4-MIGRATE负责所有生产actor接线并保留可核验证据。
...
- terminal close artifact冻结每个current contributor fence以及reason/requested-at/hard-crash cycle token budget；只有`open → closing`可创建快照，同command ID+同request重放返回既有结果，任何第二个command不得重写快照或把`draining`退回`closing`。learner最多完成当前cycle并在drain ack声明final cycle sequence/update ID；若final receipt声明`proposal_expected=true`，ack必须携带相同planned update，否则明确拒绝并保留hard-crash处置路径。drain只接受这些pre-close fence的contiguous receipt和matching final proposal，超出声明或来自new/stale attempt的一律拒绝；hard-crash contributor按per-incarnation gap上界终结；
