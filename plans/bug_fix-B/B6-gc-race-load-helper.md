...
- prediction 的 `prepare_prediction_or_find_newer_latest` 重构为该 helper 的调用方，其专属逻辑（预测分支选择）保留在外层；
- rebase 路径注意：竞态恢复拿到更新版本时，rebase 数学（reference 与新 global 的差）语义不变——rebase 本就是"搬到更新的 global 上"；但事件中的版本字段必须反映实际加载版本。

## 3. 目标与完成谓词

1. 三个未防护加载点全部经由 helper（GCR-05 静态检查：learner 运行路径上对 `load_global_weights_into_model`/`load_global_weights_flat` 的直接调用只存在于 helper 内部）；
...
- **范围内**：helper、三个加载点接入、prediction 路径重构为调用方、竞态注入测试。
- **非目标**：
  - GC 侧的发布宽限调整（GC 语义不动，防护做在读侧）；
  - fragment 加载路径——SPECIFY 阶段盘点 fragment adoption 是否有同类未防护点：有则列入本计划同批处理，无则记录排除证据（review 未逐点核查 fragment 侧，这里补上）；
...
| L2 三点接入 | 各点竞态注入测试先 RED（当前直接 FileNotFoundError 崩溃即 RED 证据） | adopt_global、rebase 加载、resume 初次加载接入 helper | GCR-01 轨迹等价；全量 pytest |
...
| GCR-05 | 静态检查 | 运行路径上权重加载直接调用仅存在于 helper 内部 |
