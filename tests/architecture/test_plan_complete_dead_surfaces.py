...
def test_early_development_layout_fallbacks_are_deleted() -> None:
    syncer = (ROOT / "fs_diloco/runtime/syncer_v4.py").read_text(encoding="utf-8")
    admission = (ROOT / "fs_diloco/storage/admission.py").read_text(encoding="utf-8")

    assert "legacy_path" not in syncer
    assert "canonical_receipt_id" not in syncer
    assert "early P4 development layout" not in admission
    assert "admissions_v4/{learner_id}" not in admission
