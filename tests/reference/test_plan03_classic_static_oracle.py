...
def _semantic_projection(store: Any, tape: EventTape, *, ha: bool) -> dict[str, Any]:
    initialization = (
        {
            "publication_id": "publication-v0",
            "weight_size_bytes": 12,
            "optim_size_bytes": 24,
        }
        if ha
        else {}
    ...
    for metadata, _vector in tape.proposals:
    ...
    vector_by_id = {metadata["update_id"]: vector for metadata, vector in tape.proposals}
    merged = weighted_average_tensors(
        [vector_by_id[row["update_id"]] for row in selected],
        [weights[row["update_id"]] for row in selected],
    )
    state0 = init_outer_state(tape.theta_v0, tape.outer)
    theta_v1, state1 = outer_optimizer_step(
        tape.theta_v0,
        tape.theta_v0 - merged,
        state0,
        tape.outer,
    )
    ...
    return {
        ...
        "merged": merged,
        "theta": theta_v1,
        "outer_step": state1["step"],
        "outer_momentum": state1["momentum"],
        "version": int(committed["version"]),
        "predecessor_version": int(committed["version"]) - 1,
    }
...
def test_classic_and_static_ha_share_exact_semantic_projection(tmp_path: Path) -> None:
    ...
    try:
        classic_projection = _semantic_projection(classic, tape, ha=False)
        ha_projection = _semantic_projection(fenced.bind(token), tape, ha=True)
        ...
        actual_fixture = {
            "selected_ids": list(classic_projection["selected_ids"]),
            "selected_order": list(classic_projection["selected_order"]),
            "weights": list(classic_projection["weights"]),
            "merged_float32_le_hex": tensor_hex(classic_projection["merged"]),
            "theta_float32_le_hex": tensor_hex(classic_projection["theta"]),
            "outer_step": int(classic_projection["outer_step"].item()),
            "outer_momentum_float32_le_hex": tensor_hex(classic_projection["outer_momentum"]),
            "version": classic_projection["version"],
            "predecessor_version": classic_projection["predecessor_version"],
        }
        assert actual_fixture == classic_fixture
    finally:
        classic.close()
        fenced.close()
        lease.close()
