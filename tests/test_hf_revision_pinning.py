...
def test_hf_model_and_tokenizer_load_the_declared_revisions(
    ...
    load_causal_lm_and_tokenizer(config, require_frozen_identity=True)
...
def test_hf_dataset_primary_and_wikitext_fallback_use_the_declared_revision(
    ...
    assert load_text_split(config, "train", require_frozen_identity=True) is expected
...
def test_hf_dataset_loader_rejects_an_existing_local_reference_or_override(
    ...
    with pytest.raises(ValueError, match="local dataset reference.*content identity"):
        load_text_split(config, "train", require_frozen_identity=True)


def test_hf_model_loader_preserves_query_and_baseline_local_compatibility(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    local_model = tmp_path / "local-model"
    ...
    class AutoTokenizer:
        @staticmethod
        def from_pretrained(name: str, **_kwargs: object) -> Tokenizer:
...
def test_hf_dataset_loader_preserves_query_and_baseline_local_compatibility(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    def load_dataset(name: str, _config_name: str | None, **_kwargs: object) -> object:
        assert name == str(local_dataset)
        return expected
