...
### 2026-08-07 00:22 JST — Phase 2 formal G8 attempt on commit b2b4e22
...
## 2026-08-07 02:07 JST — plan-complete analysis read-only RED

- Experiment `plan02-plan-complete-analysis-ro-red`, consecutive failure 1. Static PBS validation passed `bash -n scripts/miyabi/*.pbs`; all scripts use literal `group_list=xg24i002`. Submitted `qsub -l walltime=00:00:20 -v PROJECT_ROOT=/work/xg24i002/x10041/fsb_decoupled_diloco,TEST_TARGET=tests/test_fragment_analysis.py::test_analysis_opens_authority_database_query_only scripts/miyabi/run_plan02_phase2_tests.pbs` as PBS `2501969.opbs` on `mg0008`.
- Expected: the analysis module exposes and calls the enforced read-only opener, the connection reports `PRAGMA query_only=1`, an attempted DDL write raises `sqlite3.OperationalError`, and normal summary reads continue.
- Actual: the pre-fix target failed in `0.17s` at `monkeypatch.setattr(analysis_runtime, "open_readonly", ...)` because `fs_diloco.tools.analysis` has no such symbol; it still calls raw `sqlite3.connect(path)`. Log: `fsdiloco_plan02_p2_tests.o2501969`.
- Confirmed cause: `_db_summary()` bypasses `storage.schema_bootstrap.open_readonly()` and therefore opens the authority path read/write/create. Next change: import and call the shared opener, then rerun the focused analysis file and the complete test tree on a compute node.

## 2026-08-07 02:10 JST — Phase 2 incremental-review cleanup/observation RED

- Experiment `plan02-phase2-incremental-review-red`, consecutive failure 1. After `bash -n scripts/miyabi/*.pbs`, literal `group_list=xg24i002` validation and `git diff --check`, PBS `2501974.opbs` ran `tests/test_clean_run.py tests/test_plan02_phase2_review_remediation.py` on `mg0001` with a `00:00:25` request. Result: `4 failed, 14 passed in 3.98s`; log `fsdiloco_plan02_p2_tests.o2501974`. No cleanup deletion was requested or executed.
- Expected: the cleanup inventory preserves fsync-before-prune `metrics/update_history.jsonl`; direct PASS evidence is rejected unless its terminal final version matches the current summary; and an atomic merge observation is rejected when its contributor IDs/count differ from the selected updates. Actual: the cleaner selected the update archive for deletion, accepted both missing and stale terminal bindings, and accepted the first contributor-mismatched merge observation.
- Confirmed causes: `build_cleanup_plan()` misclassifies the authoritative update archive as raw telemetry; `_matching_pass_evidence()` compares terminal versions only for matched-branch evidence; and `commit_full_merge()` validates observation kind/key/version but not its contributor payload. Next change: preserve the update archive, fail closed on missing/mismatched direct terminal bindings, validate exact contributor identities/count at the fenced merge boundary, update cleanup documentation and add an append-only record of the already nonrecoverable main-G9 archive deletion. Rerun the complete cleanup/remediation group, then the full repository suite.

## 2026-08-07 02:08 JST — incremental external reviewer session limit

- Fresh canonical `claude-opus-5` invocation for base `7feb09992b7f40b255e0858020a50d811a602b9c` and target `180a243256798565bafd385467830a66b8d724c9` used session `cb74bf44-a4c9-495d-b743-bcdf42c8b90e` with the required read-only review prompt.
- The JSON result reported HTTP 429, zero model usage/cost, and `You've hit your session limit · resets 6am (Asia/Tokyo)`. Disposition: `skipped-session-limit`; no retry and no Claude report was fabricated. Per `plans/AGENTS.md`, this does not block the independently saved Codex review and remediation.
