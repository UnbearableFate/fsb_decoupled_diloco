...
## 2026-08-10 00:26 JST — P6 G5 attempt 4 exposed bootstrap scale-out and replacement-startup races

- Experiment ID `P6-G5-tiny-attempt4`，三连失败全面审查和scheduler-backed拓扑重写后的第一个运行。命令为`qsub -l walltime=00:20:00 -v PROJECT_ROOT=/work/xg24i002/x10041/fsb_decoupled_diloco,RUNS_ROOT=/work/xg24i002/x10041/fsb_decoupled_diloco/runs/fs_diloco,EVIDENCE_OUTPUT=.../artifacts/20260810-002300_p6-g5-tiny-scenarios_pass.json scripts/miyabi/run_plan03_phase6_tiny_scenarios.pbs`；parent job `2512873.opbs`在`mg0006`运行`00:03:11`后exit 1，source target `488209ca8865388c90c5fd597769625023281cdd`。4个static场景再次通过；第五个dynamic replacement在等待production child `2512883` admission时超时。原始parent日志`fsdiloco_p6_g5.o2512873`，现场run/log为`runs/fs_diloco/plan03_p6_g5_dynamic-2-learners-replacement_2512873/`和`logs/qsub_plan03_p6_g5_dynamic-2-learners-replacement_2512873/`；四份`.20260810-002300_*.partial.json`保留已通过场景。
- 预期只有parent与一个独立bootstrap/replacement child同时live；实际candidate在slot 0 bootstrap child尚未registration时即把暂缺容量解释为persistent low并额外提交scale-out `2512881`，因此短暂出现parent+bootstrap+scale-out三allocation。scale-out被occupied-stream authorization正确拒绝，却消耗launch budget。初始child `2512880`被精确qdel并有scheduler terminal证据后，production依次提交replacement `2512883/2512885/2512887`；它们均写入admitted authority row，却在首次训练进度前看到stream current-admission tombstone并以`AdmissionSupersededError`退出，随后被下一个replacement取代。最终run因4次非bootstrap launch耗尽而以`launch_budget_exhausted`在v1关闭，harness等待的第一replacement从未保持current admission。
- 已确认三个独立production缺陷。其一，capacity service没有在`membership.initial_membership_deadline_seconds`内把尚未到达的声明bootstrap slot与普通scale-out缺口区分，违背冻结的最大live allocation和initial-membership语义。其二，stream在old instance退休后具有合法`tombstone` current-admission pointer；新replacement在自己的request尚未被leader处理前会先看到这个旧tombstone，而stateless reader错误地立即判作“本attempt已被supersede”并退出。随后leader仍处理已发布request并形成短暂admitted row，解释了全部三次admit→exit→replace链。`_confirmed_lost_instance`另有same-stream旧progress早于新admission时未取两者最大值的liveness缺陷。其三，`terminal_ack_ingested`把contributor的`actor_id`写入single-writer保留identity字段，DB ack已成功后仍被包装成重复的`terminal_ack_rejected`事件。
- 下一轮先增加RED回归：initial bootstrap deadline内的low windows不得scale-out、deadline后才可填缺；未处理replacement看到旧stream tombstone必须继续等待，但已经捕获fence后的revalidation看到tombstone必须fail closed；replacement的旧stream progress不得让admission立即stale；真实`ActorTelemetryWriter`必须记录一次`terminal_ack_ingested`且不产生rejected。production随后以authority creation time实行bootstrap wait、让admission reader显式区分pre-capture polling与post-capture revalidation、用admission/progress较新时间判断loss，并把telemetry字段改名为`contributor_actor_id`。G5 harness同时把replacement查询绑定到exact lost instance且要求无scale-out、总nonbootstrap launch恰为1。修订后先重跑完整G2，再提交G5 attempt 5；不得只扩大timeout或launch budget掩盖本次证据。

## 2026-08-10 00:45 JST — P6 G5 attempt 5 compared normalized and full PBS IDs literally

- Experiment ID `P6-G5-tiny-attempt5`，命令与attempt 4相同但输出为`artifacts/20260810-004200_p6-g5-tiny-scenarios_pass.json`；parent job `2513024.opbs`在`mg0006`运行`00:03:03`后exit 1，clean source target `7aff2552b254b70274877929563b3d53eb7661f9`。四个static场景再次通过。第五个dynamic场景的production行为实际成功：initial child `2513031.opbs`终结，唯一nonbootstrap request提交replacement `2513032`，同stream epoch 2的instance进入`stopped/terminal_graceful_ack`，run以`configured_target`在v11完整finalize；没有scale-out或replacement churn。
- Harness仍在`_wait_dynamic_admission(database, replacement_job_id)`中使用SQL `pbs_job_id=?`。production launch receipt按契约保存normalized `2513032`，learner instance保存PBS提供的full `2513032.opbs`；因此已有admitted/stopped row却永远不满足literal等式，120秒后错误报`dynamic child 2513032 was not admitted`。这是验证器identity normalization错误，不是production admission失败。现场：`runs/fs_diloco/plan03_p6_g5_dynamic-2-learners-replacement_2513024/`、`fsdiloco_p6_g5.o2513024`和四份`.20260810-004200_*.partial.json`。
- 下一轮只让wait helper使用production `normalize_job_id()`比较候选row，并允许轮询观察到`admitted/draining/stopped`，后续仍由exact launch request、instance ID、stream epoch、applied update和terminal状态做严格验证。先重跑G2避免harness import/collection回归，再运行attempt 6；不得改变production协议、timeout或已通过的bootstrap/admission修复。

## 2026-08-10 00:52 JST — P6 G5 attempt 6 used the wrong terminal-status oracle

- Experiment ID `P6-G5-tiny-attempt6`，parent PBS job `2513076.opbs`在`mg0006`由相同20分钟命令启动，clean source target `9a8760ff7f37d781a2693087f7e271142a8f287b`。前四个static场景通过；第五个dynamic场景也完成真实pipeline：initial child `2513083.opbs`有scheduler FINISH，production只提交replacement `2513085`，replacement同stream/higher epoch并参与到最终v11，两个current contributors均graceful terminal ack，summary为`configured_target`。Harness随后仅因要求lost row `status='revoked'`而exit 1；实际为`status='expired'`且reason=`confirmed_scheduler_terminal_after_progress_stall`。
- `expired`是已冻结并由`test_replacement_requires_stale_progress_and_terminal_scheduler_evidence`覆盖的语义：scheduler已证明job terminal且progress超期；`revoked`保留给显式revoke/operator替换路径。计划要求所有`revoked/stopped/expired`都走统一retire command，并没有要求scheduler death伪装为revoke。因此这是review文本和harness oracle错误，production不应改成`revoked`来迎合测试。现场为`runs/fs_diloco/plan03_p6_g5_dynamic-2-learners-replacement_2513076/`、`fsdiloco_p6_g5.o2513076`和四份`.20260810-005200_*.partial.json`。
- Attempts 4/5/6在第二个连续窗口再次达到三次失败。Attempt 7前必须再次停止提交，并在`code_review.md`重新审查attempt 4新增的production行为、launch→registration identity、两种terminal status和尚未运行到的第六组合故障场景。通过条件不放宽：lost必须exact `expired`+scheduler reason，replacement exact launch binding、same stream/higher epoch、至少一次applied contribution，非bootstrap launch恰1，所有scenario-owned child scheduler terminal且六个场景全部写artifact。

## 2026-08-10 01:14 JST — P6 G6 attempt 1 used a removed authority constructor argument
...
## 2026-08-10 02:05 JST — P6 post-G9 repair G0 attempt 1 omitted the accepted config projection

- Clean source target `00fe0e42d95827077fcdf803cc7985fecbcf021c`通过diff、compile、Ruff lint/44-file format、全部shell syntax、literal PBS group和P6 matrix inventory，但formal G0 artifact `artifacts/20260810-020400_p6-g0-g1-freeze-static-pass.json`因Checker返回`config-migration.full-semantic:configs/fs_diloco_tiny_ha_dynamic_acceptance.yaml`而`BLOCKED`。
- G9已将该formal config的`initial_membership_deadline_seconds`从60秒改为600秒，以容纳八个独立PBS bootstrap job的真实排队时间；Checker的`P6_ACCEPTANCE_CONFIG_PROJECTIONS`仍只登记dtype、60 inner steps和learner walltime等既有P6变化，因而把这项有依据的P6-only timeout修订误判为P4迁移漂移。其余Checker difference为空，source identity为clean。
- 修复只把exact 600秒值加入dynamic acceptance projection，使Checker继续拒绝任意其他config漂移，并保留现有extra-drift regression。随后从新的clean commit重跑完整G0和G2；不得关闭semantic checker或把formal config临时移出source identity。

## 2026-08-10 02:12 JST — P6 G10 attempt 4 ignored the classic JSONL authority archive

- Formal G10 attempt 4，PBS job `2513594.opbs`，在clean target `94d34171c8fbc450a8bffee50c9c6deb862d041f`成功通过peeled-tag worktree、双frozen-lock venv、neutral-cwd package origin和Torch版本preflight；随后第一个classic warmup的三个actor均clean exit，harness在timer结束后的summary projection报`classic applied-update projection disagrees with global token total`并exit 1。未开始任何measured pair，未生成comparison artifact；raw log为`fsdiloco_p6_g10.o2513594`。
- 冻结classic的`run_maintenance -> archive_and_prune`会先把已裁决update追加到`metrics/update_history.jsonl`、把历史version追加到`metrics/global_version_history.jsonl`，fsync后再从hot SQLite删除；latest committed version的`total_seen_tokens`则是全程累计值。Harness只查询hot `updates/global_versions`，因此在正常维护后的warmup中只看到保留的hot子集，却把它与累计token总量比较。这不是`tokens_this_update`语义错误，而是summary遗漏了classic的正式authority archive。
- 下一步让summary按`update_id/version`合并JSONL archive与hot SQLite，拒绝冲突重复，并同时验证version 0..2连续、逐version `num_updates/total_update_tokens`、全程update token和latest `total_seen_tokens`一致，再从组合update恢复cursor identity。补一个hot只保留v2、JSONL保留v0/v1及四个applied update的detached classic fixture回归；完整G2通过后再进行G10 attempt 5。20 pairs、timer anchor、10%门槛、bootstrap CI和两个comparison均保持不变。

## 2026-08-10 02:33 JST — P6 G10 attempt 5 allowed timing to choose the measured data cursor
...
## 2026-08-10 03:12 JST — G10 remediation G2 attempt 1 exposed a deeper authority-to-Torch import edge

- Experiment ID `P6-G10-remediation-G2-attempt1`, PBS job `2513720.opbs`, one `debug-g` node, 10-minute walltime, clean target `8aaa7b7f48c6edb955178fa618a37759bd7527b0`. Focused completed `623 passed, 1 failed` in `41.91s`; full completed `739 passed, 1 failed, 2 skipped` in `86.16s`. The sole failure in both was the new fresh-interpreter probe: importing `fs_diloco.runtime.syncer_v4` found `torch` already in `sys.modules`. Raw logs: `logs/qsub_plan03_p6_g2_20260810_031300/` and `fsdiloco_p6_g2.o2513720`; structured artifact `artifacts/20260810-031300_p6-g2-tests-pass.json` has terminal status `BLOCKED` and exact JUnit metrics.
- Confirmed import chain: `syncer_v4 -> storage.authority -> storage.object_store -> torch`. `LeaderAuthority` imports the two tensor-verification functions at module scope even though admission, leases, membership initialization and all admission transactions do not call them. The syncer entrypoint also imports `LeaderAuthority` before pre-admission, so merely deferring imports in `syncer_v4` cannot overlap learner startup.
- Next change makes `storage.authority` import `verify_proposal_payload` only inside `ingest_proposal()` after its command-replay fast path, and `verify_publication_artifact` only inside prepared-publication verification. This preserves exact tensor verification before any proposal/publication commit while keeping authority construction and admission Torch-free. Extend the subprocess probe to import `syncer_entrypoint`, `syncer_v4`, and `storage.authority`; then rerun full G0/G2 before any diagnostic or formal performance run.

## 2026-08-10 03:25 JST — P6 G10 attempt 7 proved the frozen classic also overruns the target workload

- Formal G10 attempt 7, PBS job `2513766.opbs` on `mg0009`, clean target `fea42175e1e7d661b7890a9d8142b657f7811ee8`, 30-minute walltime. Environment/source probes passed and classic-vs-unified completed both warmups plus all twenty pairs, then workload-repeat validation blocked before statistics; static-vs-dynamic did not start. Artifact: `artifacts/20260810-032300_p6-g10-classic-performance-pass.json` (terminal status `BLOCKED`); raw log: `fsdiloco_p6_g10.o2513766`.
- The corrected terminal-work oracle proves the new unified target-await fix is stable in every measured repeat at 256 processed/direct tokens, cursor `[4,4]`, four selected updates and final v2. Frozen classic, however, produced five actual terminal-work variants: 256/`[4,4]`, 320/`[4,6]`, 320/`[6,4]`, 352/`[6,5]`, and 384/`[6,6]`, while its selected projection stayed 256/`[4,4]`. Its `global_only` learner checks a newer latest before terminal inside the post-publish wait and has no target-aware await state; after observing v2 it can start part or all of cycle 3 before the classic syncer publishes terminal. Increasing the poll/wait cannot close this ordering race because latest is deliberately checked first.
- The frozen tag cannot be modified. The next benchmark adapter sets only classic to its existing `local_or_global` completion with exact `max_local_steps=4`, causing its two two-step cycles to be the actual horizon; unified remains `global_only` so it stays alive for the mandatory v4 drain ack. This is an implementation-specific termination adapter for the same actual cursor/token workload, not a removal of candidate terminal cost. The workload identity will record the common exact horizon, and tests will reject any classic cursor beyond four or any selected/terminal mismatch.
- A separate observability defect remains: although `_run_arm()` now harvested event tapes, the post-trial exception escaped `run()` and `main()` replaced the payload with a sparse error artifact, discarding trials/timings/events. Before the next formal run, make post-execution validation failures return a structured `BLOCKED` result containing all completed trials and variants; preserve scratch cleanup. Then run clean G0/G2. Initializer/performance optimization remains undecided because attempt 7 correctly produced no comparable timing statistic.

## 2026-08-10 03:29 JST — classic-adapter focused static attempt 1 required formatting

- Ruff lint passed for the classic exact-horizon adapter, retained-failure-evidence path and tests. `ruff format --check` then reported only `scripts/miyabi/plan03_p6_performance.py` would be reformatted; py_compile/diff were not reached in the chained command. No runtime behavior was tested.
- Apply the formatter mechanically and rerun the same complete focused group before freezing the next target. No performance rule or workload field changes as a consequence.
## 2026-08-10 03:52 JST — P6 final-common-target G0/G1 attempt 1 BLOCKED

- Experiment ID `P6-G0-G1-final-common-target-attempt1`. Login-node static command: `/work/xg24i002/x10041/fsb_decoupled_diloco/.venv/bin/python scripts/miyabi/plan03_p6_test_gate.py --mode static --project-root /work/xg24i002/x10041/plan03-p6-final-320d --python /work/xg24i002/x10041/fsb_decoupled_diloco/.venv/bin/python --ruff /work/xg24i002/x10041/fsb_decoupled_diloco/.venv/bin/ruff --log-root /work/xg24i002/x10041/fsb_decoupled_diloco/logs/plan03_p6_g0_20260810_035200 --output /work/xg24i002/x10041/fsb_decoupled_diloco/reports/DOING/fsb_decoupled_diloco_plan_03_unified_ha/artifacts/20260810-035200_p6-g0-g1-final-pass.json`; frozen detached target `320d74d0fee41ddf0c8f6a6634f7b1db34fb00a6`, login host environment, no PBS job/run ID.
- Expected all static checks and source freeze to pass. The runner stopped before executing checks with `FileNotFoundError` for `/work/xg24i002/x10041/plan03-p6-final-320d/uv.lock`; retained structured artifact: `artifacts/20260810-035200_p6-g0-g1-final-fail.json` (`status=BLOCKED`).
- Confirmed cause: `uv.lock` is an intentionally untracked/ignored environment lock in the primary working copy and therefore was not materialized by `git worktree add`, while the source-identity contract requires its bytes. This is a frozen-worktree bootstrap omission, not a product or static-check failure.
- Before attempt 2, materialize the same lockfile inode in the detached worktree without changing its content, verify its SHA-256 matches the primary copy, then rerun the identical static gate. Success requires clean source identity for `320d74d`, all checks return zero and the seven P6 requirement rows plus frozen 19-job cost record are present.

## 2026-08-10 03:54 JST — P6 final-common-target G5 attempt 1 FAIL

- Experiment ID `P6-G5-final-common-target-attempt1`. After all PBS scripts passed `bash -n` and literal `xg24i002` group checks, command `qsub -l walltime=00:20:00 -v PROJECT_ROOT=/work/xg24i002/x10041/plan03-p6-final-320d,PRIMARY_WORKTREE_ROOT=/work/xg24i002/x10041/fsb_decoupled_diloco,RUNS_ROOT=/work/xg24i002/x10041/fsb_decoupled_diloco/runs/fs_diloco,PYTHON_BIN=/work/xg24i002/x10041/fsb_decoupled_diloco/.venv/bin/python,EVIDENCE_OUTPUT=.../artifacts/20260810-035600_p6-g5-final-pass.json scripts/miyabi/run_plan03_phase6_tiny_scenarios.pbs` ran as PBS job `2513857.opbs` on `mg0014`, clean frozen source target `320d74d0fee41ddf0c8f6a6634f7b1db34fb00a6`; requested walltime `00:20:00`, actual `00:00:01`, exit 1.
- Expected all six real tiny topologies to execute. The first scenario failed before actor startup because the harness deliberately invokes `<project-root>/.venv/bin/python`, but a newly created detached Git worktree contains no ignored `.venv`; minimal symptom: `FileNotFoundError: /work/xg24i002/x10041/plan03-p6-final-320d/.venv/bin/python`. No structured artifact or run evidence was produced. Raw log: `/work/xg24i002/x10041/plan03-p6-final-320d/fsdiloco_p6_g5.o2513857`.
- Confirmed cause is incomplete frozen-worktree environment bootstrap, not a topology/runtime failure. Before attempt 2, bind the detached worktree's ignored `.venv` path to the same immutable primary environment, verify Python identity/imports and repeat the unchanged six-scenario gate. Success still requires all six scenarios and their complete DB/terminal/accounting checks on `320d74d`.

## 2026-08-10 03:59 JST — P6 quality-boundary attempt 1 BLOCKED

- Experiment ID `P6-QUALITY-final-attempt1`. Login-node manifest command: `/work/xg24i002/x10041/fsb_decoupled_diloco/.venv/bin/python scripts/miyabi/plan03_p6_quality_manifest.py --project-root /work/xg24i002/x10041/plan03-p6-final-320d --g2 .../artifacts/20260810-035600_p6-g2-final-pass.json --output .../artifacts/20260810-035900_p6-quality-final-pass.json`; frozen clean source target `320d74d0fee41ddf0c8f6a6634f7b1db34fb00a6`, no PBS job/run ID.
- Expected a non-blocking quality-boundary manifest backed by the passing deterministic/numeric/baseline tests in G2. It returned `BLOCKED` because it requires `tests/reference/test_plan03_classic_static_oracle.py`, a test deliberately deleted in P5 with the classic writer. Retained artifact: `artifacts/20260810-035900_p6-quality-final-fail.json`.
- Confirmed cause: the newly added P6 manifest encoded the pre-P5 owner path instead of the retained post-deletion contract. The frozen classic/static oracle remains as immutable golden fixtures and P0 evidence, while current comparability behavior is owned by `tests/gates/test_plan03_p6_performance_harness.py`; current numeric and baseline owners remain present and passed in G2.
- Replace the deleted path with the retained performance/comparability harness and make the manifest explicitly name the immutable P0 classic/static oracle artifact rather than pretending deleted production-oracle code is still runnable. Add a regression for the manifest owner inventory, then rerun static/G2 on a new clean target and regenerate this manifest. The three-seed quality study remains `NOT_RUN` and non-blocking; no quality claim changes.

## 2026-08-10 04:10 JST — P6 final evidence cleanup attempt 1 refused all safe cleanup

- Experiment ID `P6-FINAL-CLEANUP-attempt1`. After the final G8/G9 PASS artifacts were persisted, login-node dry runs invoked `.venv/bin/python -m fs_diloco.tools.clean_run` for the exact completed roots `runs/fs_diloco/plan03_p6_g8_static_2513862` and `runs/fs_diloco/plan03_p6_g9_dynamic_2513863`, bound respectively to `artifacts/20260810-035600_p6-g8-final-pass.json` and `artifacts/20260810-035600_p6-g9-final-pass.json`. No deletion was requested or performed.
- Both dry runs returned code 2 before producing an inventory: G8 reported `authority still references cleanup candidate: updates/payloads/learner_000/0cf36abb-5a6c-4dff-bc6c-c54ac09b6c87.safetensors`; G9 reported the same condition for `updates/payloads/0/02f9bae6-48e3-4a8e-b27f-16c438f831e1.safetensors`.
- Read-only DB inspection confirms neither run has pending/selected updates: each retains eight `applied` rows only. The reported paths are instead `gc_candidates.state='pending'`, created when schema-9 maintenance archived exact history; generic cleanup correctly must not delete authority-owned claims. The defect is that `clean_run` combines active recovery references and already-claimed-for-fenced-GC references into one set, then aborts the entire plan. It therefore cannot prune unrelated terminal heartbeat/pointer/telemetry objects, and cannot emit an auditable retained-path inventory.
- Add a RED regression for a completed run containing one authority-owned pending GC payload plus unrelated cleanup-safe terminal files. Preserve/refuse active `updates`, prepared/committed/orphan publication references and claimed objects exactly as before, but omit pending/claimed `gc_candidates` from deletion candidates and record them explicitly as retained authority-owned paths. Then rerun the full cleanup test group/G2 and repeat dry-run→delete only on exact evidence-bound roots. No authority DB, GC-owned object, checkpoint, audit or user file may be mutated by the fix.

## 2026-08-10 04:15 JST — P6 cleanup remediation focused static attempt 1 required formatting

- Experiment ID `P6-FINAL-CLEANUP-focused-static-attempt1`. `.venv/bin/ruff check fs_diloco/tools/clean_run.py tests/test_clean_run.py` passed, while the immediately following `ruff format --check` reported both edited files would be reformatted; no pytest or Torch workload ran on the login node.
- The new behavior keeps active update/publication references fail-closed, records pending/claimed `gc_candidates` as authority-owned retained paths, and allows unrelated terminal-only objects into the deletion inventory. The failure is canonical formatting only, not a semantic test result.
- Apply the repository formatter mechanically to those two files, rerun the complete focused Ruff/format/compile/diff group, and defer runtime regression to the required PBS G2 target.

## 2026-08-10 04:17 JST — G10 result projection diagnostic attempt 1 used the wrong import module

- Experiment ID `P6-G10-final-result-projection-attempt1`. A read-only login-node one-liner intended to summarize the two completed G10 JSON artifacts wrote `from pathlib import Path,Counter`; it stopped with `ImportError: cannot import name 'Counter' from 'pathlib'` before reading either artifact. No source, report artifact or run data was changed.
- This is only a diagnostic command typo: `Counter` belongs to `collections`. The PBS stdout already ends with two `PASS` lines and both structured artifacts exist. Retry the read-only projection with `from collections import Counter`; formal G10 itself is not rerun and its failure count is unaffected.

## 2026-08-10 04:21 JST — P6 G0-G7 aggregate attempt 1 hard-coded the wrong requirement count

- Experiment ID `P6-G0-G7-final-aggregate-attempt1`. The login-node JSON aggregator consumed the seven final PASS inputs on the exact clean source target `320d74d0fee41ddf0c8f6a6634f7b1db34fb00a6` and wrote `artifacts/20260810-042100_p6-g0-g7-final-pass.json`, but returned `BLOCKED` with the sole error `G0 P6 requirement matrix is incomplete`; every input gate status is `PASS` and the common commit check passed.
- Read-only inspection shows the G0 artifact correctly freezes all eight P6 phase rows: `AUTH-11` plus the seven `P6-*` rows. The new aggregator incorrectly requires exactly seven rows, omitting the cross-phase-named `AUTH-11` row even though its phase is `P6-acceptance-final-review`. This is an aggregate-validator defect, not missing acceptance evidence.
- Replace the cardinality-only assertion with an exact expected requirement-ID set containing `AUTH-11`, `P6-ACCEPTANCE`, `P6-STATIC-9N`, `P6-DYNAMIC-9N`, `P6-PERF-CLASSIC`, `P6-PERF-DYNAMIC`, `P6-QUALITY` and `P6-DOCS`; add a regression that rejects both omissions and extras. Run the aggregate/checker tests in PBS G2, then regenerate the aggregate from the unchanged seven PASS inputs. Preserve this attempt as a `fail` artifact rather than overwriting it.

## 2026-08-10 04:23 JST — P6 aggregate remediation focused static attempt 1 required test formatting
...
## 2026-08-10 04:42 JST — P6 phase-review ancestry preflight attempt 1 used an unverified target hash

- Experiment ID `P6-PHASE-REVIEW-ancestry-preflight-attempt1`. A read-only `git merge-base --is-ancestor`/diff inventory command used the mistyped full target `e3a89c7d14f6b301c45a5fd25ec93d6be3d9ce42` inferred from the short commit display and failed with `Not a valid commit name`; no review report or repository state was changed.
- `git rev-parse HEAD` gives the actual frozen target `e3a89c784005acb6c92603d6fea24170bea4daf5`; the declared phase base remains `7f797e47d983878e25f9c48c1fddbeb9f0b2ea4f`. Retry ancestry and diff inventory with those verified full IDs before writing either independent review. This does not count as a code-review finding or invalidate prior gates.

## 2026-08-10 04:57 JST — P6 phase-review H1 RED proves online archive removes a still-current receipt

- Experiment ID `P6-H1-archive-current-receipt-RED-attempt1`. After `bash -n scripts/miyabi/*.pbs` and the literal `group_list=xg24i002` scan passed, command `qsub -l walltime=00:10:00 -v PROJECT_ROOT=/work/xg24i002/x10041/fsb_decoupled_diloco,EVIDENCE_OUTPUT=/work/xg24i002/x10041/fsb_decoupled_diloco/reports/DOING/fsb_decoupled_diloco_plan_03_unified_ha/artifacts/20260810-051500_p6-h1-archive-current-receipt-red-fail.json scripts/miyabi/run_plan03_phase6_tests.pbs` ran as PBS job `2514056.opbs` on one `debug-g` node. The tracked source target was `e3a89c784005acb6c92603d6fea24170bea4daf5`; the only uncommitted executable change was the new RED test, which the old source-capture dirtiness scope incorrectly reported as clean and thereby independently confirms review finding M4.
- Expected behavior: while the controller is open, archiving history through v1 must retain learner-0's cycle-1 receipt because `contributor_progress.last_receipt_id` still names it; after both contributors acknowledge and the controller finalizes, the same receipt may become archivable. Actual behavior: the first archive deleted the hot row, so `SELECT COUNT(*) FROM cycle_receipts WHERE receipt_id=?` returned `0` instead of `1`. Focused result was `626 passed, 1 failed`; full result was `745 passed, 1 failed, 2 skipped`. Raw logs are `logs/qsub_plan03_p6_g2_2514056/{collect,focused,full}.log` and `fsdiloco_p6_g2.o2514056`; the structured artifact is `artifacts/20260810-051500_p6-h1-archive-current-receipt-red-fail.json`.
- Confirmed cause: `_audit_history_records()` admits batches and updates whose receipt is still the contributor's current progress anchor. It therefore removes the receipt before `acknowledge_terminal_contributor()` can verify a clean final cycle. The fix will protect progress-anchored receipts, their updates, and their selection-batch dependency closure in all nonterminal controller states, while allowing the terminal force-maintenance pass to archive them after state `finalized`/`error`. The same RED test will prove both halves. The next PBS G2 must pass 627 focused and 746 full tests (plus the two established skips), with zero failures/errors.

## 2026-08-10 05:03 JST — P6 review-remediation G2 attempt 1 was source-unstable and exposed an incomplete config deletion projection

- Experiment ID `P6-phase-review-remediation-G2-attempt1`, PBS job `2514142.opbs`, one `debug-g` node, requested walltime `00:10:00`. Command: `qsub -l walltime=00:10:00 -v PROJECT_ROOT=/work/xg24i002/x10041/fsb_decoupled_diloco,EVIDENCE_OUTPUT=.../artifacts/20260810-052000_p6-phase-review-remediation-g2-pass.json scripts/miyabi/run_plan03_phase6_tests.pbs`. Focused result was `627 passed, 1 failed`; the later full process saw a newer working tree and returned `748 passed, 2 skipped`. The terminal `BLOCKED` artifact is retained as `artifacts/20260810-052000_p6-phase-review-remediation-g2-fail.json`; raw logs are `logs/qsub_plan03_p6_g2_2514142/` and `fsdiloco_p6_g2.o2514142`.
- The focused failure was `test_plan03_p4_semantic_migration_accepts_p5_deletions`: removal of the now-confirmed dead `syncer.parallel_checkpoint_writes` field from both acceptance configs was not represented in the checker projection, so both configs were reported as semantic drift. The test process correctly rejected an unrecorded deletion.
- This run is additionally invalid as a remediation PASS because implementation edits continued after submission; collection, focused, full and source capture did not observe one frozen tree. It must not be cited as positive evidence even though the full subprocess happened to pass. This sequencing error is confined to validation orchestration and made no run-data mutation beyond this disposable test output.
- The checker now has an explicit exact-path removal projection for only `syncer.parallel_checkpoint_writes` in the two P6 acceptance configs; all source, schema, config, harness and tests are frozen before the next submission. Rerun the same PBS G2 only after focused Ruff/format/diff checks, and require both focused and full suites to pass from one unchanged source fingerprint.

## 2026-08-10 05:08 JST — P6 review-remediation G2 attempt 2 passed tests and correctly blocked an uncommitted source

- Experiment ID `P6-phase-review-remediation-G2-attempt2`, PBS job `2514150.opbs`, one `debug-g` node, requested walltime `00:10:00`. The unchanged submission requested output `artifacts/20260810-050500_p6-phase-review-remediation-g2-pass.json`; its terminal `BLOCKED` artifact is retained as `artifacts/20260810-050500_p6-phase-review-remediation-g2-fail.json`. Preflight `git diff --check`, compileall, full Ruff, changed-file format, PBS syntax and literal group scans all passed.
- The source stayed frozen throughout this job. Focused completed `628 passed` and full completed `748 passed, 2 skipped`, with zero failures/errors. The new archive/current-receipt regression, source-scope regression, primary G7 AUTH-11 ownership regression, dynamic-schema cleanup and config-deletion projection all passed.
- The structured gate status is intentionally `BLOCKED` with the sole error `formal executable source scope is dirty`: review remediation was not yet committed, and the corrected `EXECUTABLE_SOURCE_SCOPES` now detects the changed tests as well as source/config/scripts. This is the expected fail-closed behavior for M4, not a behavior-test failure, but it cannot serve as final G2 evidence.
- Freeze the reviewed remediation in a new commit without `plans/AGENTS.md`, perform the required incremental phase review because H1 changes a persistence invariant, then rerun G2 and the affected/full P6 acceptance ladder on one clean target. PASS requires the same test metrics plus `git_dirty=false` and a source fingerprint bound to that target.

## 2026-08-10 05:35 JST — P6 incremental-review RED proves the formal producer scope and historical P6 config projection defects

- Experiment ID `P6-incremental-review-RED-attempt1`. After every `scripts/miyabi/*.pbs` file passed `bash -n` and the literal `group_list=xg24i002` scan, command `qsub -l walltime=00:10:00 -v PROJECT_ROOT=/work/xg24i002/x10041/fsb_decoupled_diloco,EVIDENCE_OUTPUT=/work/xg24i002/x10041/fsb_decoupled_diloco/reports/DOING/fsb_decoupled_diloco_plan_03_unified_ha/artifacts/20260810-053235_p6-incremental-review-red-pass.json scripts/miyabi/run_plan03_phase6_tests.pbs` ran as PBS job `2514217.opbs` on one `debug-g` node. The committed target was `557874c1761e10dcc0243f0f315742b386d553d8`; only the two new RED tests and review/report files were uncommitted. Retained structured artifact: `artifacts/20260810-053235_p6-incremental-review-red-fail.json`; complete raw failure logs: `logs/qsub_plan03_p6_g2_2514217/{collect,focused,full}.log`, JUnit XML beside them, and `fsdiloco_p6_g2.o2514217`.
- Expected behavior: the canonical formal source manifest includes tracked `tests/`, `docs/`, and `main.py`, and modifications in each change both `git_dirty` and the fingerprint; a pre-removal v4 resolved snapshot containing only `syncer.parallel_checkpoint_writes` as a retired field is strictly rejected with the registered removal diagnostic but remains readable through `load_query_config_snapshot`. Actual behavior: the manifest omitted all three paths, and the strict loader emitted `unknown config key: syncer.parallel_checkpoint_writes`; query projection could not be reached. Focused tests were unchanged and passed `628`; the full suite produced exactly `747 passed, 2 failed, 2 skipped` (`751` collected).
- Confirmed causes match Claude incremental findings H1 and M1. Every formal G0–G10 producer uses `capture_source_identity.SOURCE_SCOPES`, while the prior change altered only the requirement checker's separate tuple; and the removed syncer field was added neither to `REMOVED_CONFIG_KEYS` nor to the query-only compatibility projection. The artifact itself reports `git_dirty=false` and the old six scopes despite the modified RED tests, directly proving the attestation hole.
- Next modification: make `capture_source_identity.SOURCE_SCOPES` the single shared formal scope, including `fs_diloco`, `tests`, `configs`, `scripts`, `docs`, `main.py` and environment lock/version files; have the requirement checker and G7 producer consume that definition. Register the retired syncer key, project only that exact field for read-only historical queries while leaving runtime/resume loaders strict, and document the boundary. The next PBS G2 must pass both REDs, all focused tests and the full suite with zero failures/errors; a later clean-target gate must additionally prove the expanded scope and fingerprint in the retained artifact.

## 2026-08-10 05:42 JST — P6 incremental-review remediation G2 attempt 1 exposed removal/projection ordering

- Experiment ID `P6-incremental-review-remediation-G2-attempt1`. With PBS syntax and literal group checks already passing, command `qsub -l walltime=00:10:00 -v PROJECT_ROOT=/work/xg24i002/x10041/fsb_decoupled_diloco,EVIDENCE_OUTPUT=/work/xg24i002/x10041/fsb_decoupled_diloco/reports/DOING/fsb_decoupled_diloco_plan_03_unified_ha/artifacts/20260810-053929_p6-incremental-review-remediation-g2-pass.json scripts/miyabi/run_plan03_phase6_tests.pbs` ran as PBS job `2514234.opbs` on one `debug-g` node. Retained artifact: `artifacts/20260810-053929_p6-incremental-review-remediation-g2-fail.json`; raw logs: `logs/qsub_plan03_p6_g2_2514234/{collect,focused,full}.log`, JUnit XML, and `fsdiloco_p6_g2.o2514234`.
- Expected all behavior tests to pass and the formal gate to remain `BLOCKED` only because reviewed changes were uncommitted. Actual results were `623 passed, 5 failed` in the focused set and `744 passed, 5 failed, 2 skipped` in the full suite. All five failures entered `verify_p4_migration_contracts`, where strict removal validation raised `config removal path is absent: ('syncer', 'parallel_checkpoint_writes')`. The new source scope worked as intended: the artifact reports `git_dirty=true` and the canonical nine scopes including `tests`, `docs`, and `main.py`.
- Confirmed cause: the checker applies the P6 acceptance projection first, and that projection replaces the entire `syncer` mapping with the post-removal shape. The newly strict removal then correctly observes that its target is absent, but at the wrong stage. The removal declaration is not stale in the frozen migrated payload; the order is.
- Before another run, apply exact removals to the frozen migrated payload before applying post-P6 value projections. This preserves fail-closed detection of stale/mistyped removal paths and then permits the reviewed replacement projection. Rerun the same focused/full G2; success requires zero test failures/errors, with the only nonformal condition before commit being the expected dirty-source block.

## 2026-08-10 05:47 JST — P6 incremental-review remediation G2 attempt 2 proved the removal declaration is redundant

- Experiment ID `P6-incremental-review-remediation-G2-attempt2`. After repeated PBS syntax/literal-group and focused static checks passed, command `qsub -l walltime=00:10:00 -v PROJECT_ROOT=/work/xg24i002/x10041/fsb_decoupled_diloco,EVIDENCE_OUTPUT=/work/xg24i002/x10041/fsb_decoupled_diloco/reports/DOING/fsb_decoupled_diloco_plan_03_unified_ha/artifacts/20260810-054400_p6-incremental-review-remediation-g2-pass.json scripts/miyabi/run_plan03_phase6_tests.pbs` ran as PBS job `2514252.opbs` on one `debug-g` node. Retained artifact: `artifacts/20260810-054400_p6-incremental-review-remediation-g2-fail.json`; raw logs: `logs/qsub_plan03_p6_g2_2514252/{collect,focused,full}.log`, JUnit XML, and `fsdiloco_p6_g2.o2514252`.
- Expected the reordered removal to find the old field in the migrated frozen payload. Actual results remained `623 passed, 5 failed` focused and `744 passed, 5 failed, 2 skipped` full; all failures now report `config removal crosses non-mapping field: ('syncer', 'parallel_checkpoint_writes')` because the migrated frozen P0 payload has no `syncer` section at all.
- Confirmed root cause after comparing the immutable frozen config and the prior reviewed checker: P6 first introduced the entire `syncer` mapping through `P6_ACCEPTANCE_CONFIG_PROJECTIONS`, whose old value contained `parallel_checkpoint_writes`. The current exact whole-mapping projection already expresses the post-removal shape. A separate removal path against the pre-projection P0 payload is therefore structurally invalid, while applying it after projection is necessarily absent; there is no correct ordering for that redundant declaration.
- Before another run, delete `P6_ACCEPTANCE_CONFIG_REMOVALS` and its helper/call entirely. Keep the exact whole-`syncer` projection as the sole migration contract, which rejects any missing/extra current key by whole-payload equality and avoids the silent/stale removal API identified by review Low L1. Rerun the unchanged G2; passing tests must include the exact acceptance projection regression and both new H1/M1 REDs. This is attempt 2 of the same experiment; a third failure would trigger the mandatory full failure review before any fourth run.

## 2026-08-10 05:52 JST — P6 incremental-review remediation G2 attempt 3 passed behavior but remained nonformal on the dirty review tree

- Experiment ID `P6-incremental-review-remediation-G2-attempt3`. After deleting the redundant removal mini-language, static lint/format/diff and PBS checks passed. Command `qsub -l walltime=00:10:00 -v PROJECT_ROOT=/work/xg24i002/x10041/fsb_decoupled_diloco,EVIDENCE_OUTPUT=/work/xg24i002/x10041/fsb_decoupled_diloco/reports/DOING/fsb_decoupled_diloco_plan_03_unified_ha/artifacts/20260810-054900_p6-incremental-review-remediation-g2-pass.json scripts/miyabi/run_plan03_phase6_tests.pbs` ran as PBS job `2514273.opbs` on one `debug-g` node. Retained artifact: `artifacts/20260810-054900_p6-incremental-review-remediation-g2-fail.json`; raw logs: `logs/qsub_plan03_p6_g2_2514273/{collect,focused,full}.log`, JUnit XML, and `fsdiloco_p6_g2.o2514273`.
- The complete behavior result passed: `628 passed` focused and `749 passed, 2 skipped` full, with zero failures/errors. This includes the REDs for canonical source capture and historical removed-v4-key query projection, the exact P6 acceptance projection checks, authority persistence regressions, and all other repository tests. The structured gate nevertheless remained `BLOCKED` solely on `formal executable source scope is dirty`, as required because the reviewed remediation was not committed. It reports the intended canonical nine scopes and fingerprint `sha256:1376ea14bdb0f00096aa49331135cb528660cff3b38359f7982da56561aeb620`.
- Fact: the product/test defects from attempts 1–2 are resolved. Fact: this is still the third consecutive non-PASS result for the same G2 gate and cannot be promoted as formal evidence. Per `plans/AGENTS.md`, no fourth G2 submission is allowed until `code_review.md` records a full Codex+GPT review of all three attempts and rewrites the execution logic.
- Next step is review rather than another code tweak: trace frozen P0 config → migration → exact P6 projection → current config, and tracked/untracked source → canonical manifest → G2 artifact → acceptance/checker consumers. The revised execution boundary must commit the passing source/test tree first, exclude only report-only/user-owned changes, then run G2 on the clean frozen target; formal success requires the same zero-failure metrics plus `git_dirty=false` and a fingerprint regenerated from that target.

## 2026-08-10 06:35 JST — P6 replacement requirement staged Checker attempt 1 rejected an invalid CLI composition

- Experiment ID `P6-replacement-requirements-staged-attempt1`. On the login/control-plane node, command `.venv/bin/python scripts/miyabi/check_plan03.py --root /work/xg24i002/x10041/fsb_decoupled_diloco --expect reports/DOING/fsb_decoupled_diloco_plan_03_unified_ha/artifacts/20260808-223500_p0-runtime-surface-inventory_review.json --verify-boundaries --verify-p3-operational-contracts --verify-p5-contracts --verify-phase-requirements P6-acceptance-final-review --verification-target-ref dea559a844f898d10ab5b08ec3647e2da8c437f4 --phase P6-acceptance-final-review --mode staged --inventory-output reports/DOING/fsb_decoupled_diloco_plan_03_unified_ha/artifacts/20260810-063500_p6-requirements-staged-pass.json` exited 2 before evaluation. No artifact was created; stderr states `--phase cannot be combined with --verify-phase-requirements`.
- Expected behavior was one staged unified phase check over the newly rebound eight P6 rows. Actual behavior is only a caller error: `--phase` already selects and verifies the phase, so the lower-level `--verify-phase-requirements` switch is mutually exclusive. No source, matrix, evidence or runtime invariant was evaluated or changed.
- Next attempt will remove only `--verify-phase-requirements P6-acceptance-final-review` and retain `--phase P6-acceptance-final-review --mode staged`, all cross-phase boundary flags and target `dea559a...`. PASS requires a structured artifact with zero differences.

## 2026-08-10 06:53 JST — plan-complete remediation RED attempt 1 reproduced all four accepted findings

- Experiment ID `plan-complete-remediation-RED-attempt1`; this is consecutive failure 1 for the new remediation experiment. After all tracked PBS scripts passed `bash -n` and every literal group directive was verified as `#PBS -W group_list=xg24i002`, command `qsub -l walltime=00:10:00 -v PROJECT_ROOT=/work/xg24i002/x10041/fsb_decoupled_diloco,EVIDENCE_OUTPUT=/work/xg24i002/x10041/fsb_decoupled_diloco/reports/DOING/fsb_decoupled_diloco_plan_03_unified_ha/artifacts/20260810-065000_plan-complete-remediation-red-fail.json scripts/miyabi/run_plan03_phase6_tests.pbs` ran as PBS job `2514442.opbs` on `mg0006` with explicit ten-minute walltime. Source base was `e0f4c9b88eab4a462c49e9746d8ac5da8fb8c6b0`; only the new RED tests and review/report files were uncommitted.
- Expected behavior was that the old target fail each newly accepted contract before production remediation. Actual focused result was `644 passed, 20 failed`; full result was `767 passed, 24 failed, 2 skipped`. The 24 failures consist of three dead/compatibility surface assertions, 11 unpinned repository GPT-2 configs, six strict Full-v4 pin-validation cases, and four loader propagation cases (model/tokenizer primary/inherited revision plus dataset primary/fallback). There were zero test errors. Structured artifact: `artifacts/20260810-065000_plan-complete-remediation-red-fail.json`; complete raw logs and JUnit are `logs/qsub_plan03_p6_g2_2514442/`; PBS stdout is `fsdiloco_p6_g2.o2514442`.
- Confirmed causes are exactly Codex plan-complete findings H1/M1/M2/L1: loader kwargs omit declared revisions; Full v4 validation accepts missing/movable Hub refs and repository configs omit pins; old helper/API/layout surfaces remain; `core.constants` and fixed-path/helper surfaces duplicate current owners. The additional formal dirty-source error is expected for an uncommitted RED tree and is not a separate behavior defect.
- Next modification: pass exact revision kwargs; add Full-v4-only immutable Hub SHA validation while exempting synthetic/local and preserving baseline/query-only behavior; pin all 11 repository GPT-2/WikiText configs; remove only the proven no-caller compatibility/dead surfaces and update old tests to canonical APIs. The next PBS G2 must pass all 793 collected tests (plus any fixture updates/new count), with zero failures/errors; before the final formal run the remediated tree must be frozen in a clean commit.

## 2026-08-10 07:04 JST — plan-complete remediation G2 attempt 2 exposed two transcription errors in proposed Hub pins

- Experiment ID `plan-complete-remediation-G2-attempt2`; consecutive failure 2. With production remediation and test/config updates frozen before submission, the same validated PBS script ran as job `2514477.opbs` on `mg0005`, explicit `00:10:00` walltime, using output `artifacts/20260810-070000_plan-complete-remediation-g2-attempt2-fail.json`. Focused result was `606 passed, 58 failed`; full result was `732 passed, 58 failed, 2 skipped`; zero errors. Raw logs/JUnit: `logs/qsub_plan03_p6_g2_2514477/`; stdout: `fsdiloco_p6_g2.o2514477`.
- Every failure shares one root: the initially transcribed GPT-2 string `607a30d783dfa663caf39e06633721c8d4cfcd7eb` has 41 characters, so the new strict validator correctly rejected it. A read-only check of the existing Hugging Face cache proved the exact model ref/snapshot is the 40-character `607a30d783dfa663caf39e06633721c8d4cfcd7e`. The same cache check also found that the proposed WikiText string was shifted: the actual 40-character ref/snapshot is `b08601e04326c79dfdd32d625aee71d232d685c3`, not `08601e04326c79dfdd32d625aee71d232d685c3b`.
- Facts: loader propagation, dead-surface removal, synthetic/baseline exceptions and strict 40-lowercase-hex validation were not contradicted; execution remained source-stable. The failure came from using two malformed external identity literals in configs, tests, checker projection and remediation text. The formal dirty-source block remains expected before commit.
- Next modification: replace only the two malformed literals everywhere with the cache-verified 40-character snapshot IDs, add exact-value assertions (not length-only) for repository GPT-2/WikiText configs, rerun static checks and one final PBS G2. Attempt 3 must pass; another failure would trigger the mandatory full failure review before a fourth attempt.

## 2026-08-10 07:10 JST — plan-complete remediation G2 attempt 3 passed product changes but caught out-of-scope formatter drift

- Experiment ID `plan-complete-remediation-G2-attempt3`; consecutive failure 3. PBS job `2514493.opbs` ran on `mg0002` with explicit `00:10:00` walltime after the same complete syntax/literal-group/static preflight. Focused result was `661 passed, 3 failed`; full result was `787 passed, 3 failed, 2 skipped`; zero errors. Artifact: `artifacts/20260810-071000_plan-complete-remediation-g2-attempt3-fail.json`; raw logs/JUnit: `logs/qsub_plan03_p6_g2_2514493/`; stdout: `fsdiloco_p6_g2.o2514493`.
- All H1/M1/M2/L1 REDs and remediated behavior passed. The only three failures are different call sites asserting the same P0 boundary invariant: `verify_boundaries(...)=[]` instead returned `boundary_manifest_sha256`. Investigation showed a bulk Ruff formatting command had mechanically reformatted baseline-retention files outside the accepted finding scope, including frozen `fs_diloco/baselines/artifacts.py`, `fs_diloco/baselines/health.py`, `tests/test_torch_baseline_artifacts_and_data.py`, and `tests/test_torch_baseline_protocol.py`. The P0 manifest correctly rejected these byte changes; updating the frozen evidence would be wrong.
- No source edit occurred during the job. The separate dirty-source block is expected. Since this is the third consecutive non-PASS result, no fourth submission is allowed until the complete Codex+GPT review in `code_review.md` is saved and its revised implementation logic is followed.
- Next action after review: reverse only the mechanically introduced, out-of-scope formatting hunks back to the current HEAD bytes; preserve the intentional remediation diff; statically compare every frozen boundary path/hash and add a pre-PBS boundary-check step. Then freeze/commit before any formal G2 rerun, so the next submission is the reviewed attempt 4 rather than another dirty-tree trial.

## 2026-08-10 07:23 JST — incremental review M1 RED proves mutable local input bypass

- Experiment boundary changed after the mandatory target `397e2b7d96d339b69bbb8f6f9e78a024346786c8` incremental review found that path-prefix exemptions bypass Full-v4 content identity. New experiment ID `plan-complete-incremental-M1-RED-attempt1`; consecutive failure 1. After full PBS syntax/literal-group checks, `qsub -l walltime=00:10:00 -v PROJECT_ROOT=/work/xg24i002/x10041/fsb_decoupled_diloco,EVIDENCE_OUTPUT=/work/xg24i002/x10041/fsb_decoupled_diloco/reports/DOING/fsb_decoupled_diloco_plan_03_unified_ha/artifacts/20260810-072000_plan-complete-incremental-m1-red-attempt1-fail.json scripts/miyabi/run_plan03_phase6_tests.pbs` ran as job `2514532.opbs` on `mg0004`.
- Expected each absolute, dot-relative, parent-relative and `file://` non-synthetic model/data reference to fail closed until Full v4 has a descriptor-bound local content manifest. Actual code exempted all eight values: focused was `664 passed, 8 failed`; full was `790 passed, 8 failed, 2 skipped`; zero errors. Artifact: `artifacts/20260810-072000_plan-complete-incremental-m1-red-attempt1-fail.json`; raw logs/JUnit: `logs/qsub_plan03_p6_g2_2514532/`; stdout: `fsdiloco_p6_g2.o2514532`.
- Confirmed cause: `_is_explicit_local_reference` returns true solely from spelling and `_validate_full_input_revisions` skips revision/content validation, while the descriptor contains neither a local file manifest nor content digest. This permits actual actor bytes to change without changing run identity; `file://` is additionally not the supported local-directory spelling of the Transformers loader. All other 790 full tests passed, including the first remediation, exact pins and frozen boundary checks. The dirty-source error is expected for the uncommitted RED.
- Next modification: detect local spellings and raise an explicit unsupported-without-content-identity error in Full v4; only synthetic retains an exemption and baseline retains its separate profile. Generalize the repository-pin test so other future Hub IDs use generic SHA validation while GPT-2/WikiText keep exact expected commits; synchronize docs and rerun the same G2 experiment.

## 2026-08-10 07:30 JST — incremental review M1 attempt 2 passed behavior and stopped at the review freeze boundary

- Experiment ID `plan-complete-incremental-M1-attempt2`; consecutive non-PASS 2 for this new experiment. PBS job `2514544.opbs` ran on `mg0004`, explicit `00:10:00` walltime, from one unchanged working tree. Focused completed `673 passed`; full completed `799 passed, 2 skipped`; zero failures/errors. Artifact: `artifacts/20260810-073000_plan-complete-incremental-m1-attempt2-fail.json`; raw logs/JUnit: `logs/qsub_plan03_p6_g2_2514544/`; stdout: `fsdiloco_p6_g2.o2514544`.
- All eight local-path RED cases now fail closed with the explicit content-identity diagnostic. Other immutable Hub IDs are accepted at valid commits, repository GPT-2/WikiText exact pins remain enforced, the first remediation and the complete suite pass, and the login-safe P0 boundary/migration checker separately returns PASS.
- The structured gate is intentionally `BLOCKED` only on `formal executable source scope is dirty`, because the review remediation tests/code/docs and reports are not committed. No behavior or static-boundary defect remains. This result cannot be cited as formal G2 evidence.
- Next step is the reviewed freeze transition rather than a third dirty-tree rerun: commit the M1/L1 remediation, RED/failure artifact, skipped-Claude metadata and updated disposition while excluding user `plans/AGENTS.md`; then perform the required short incremental review on the new target. Formal attempt 3 must run from that clean approved target and return the same passing suites plus `git_dirty=false`.

## 2026-08-10 07:39 JST — second incremental review loader-local RED proves ambiguous relative/env resolution bypass

- Experiment boundary changed after target `3cd7dab0ca846a9914b8e6784c7fc11e818a7ade` review followed the string from config validation into the actual Hugging Face producers. New experiment ID `plan-complete-loader-local-RED-attempt1`; consecutive failure 1. PBS job `2514566.opbs` ran on `mg0004` with explicit `00:10:00` walltime after required syntax/literal-group checks.
- Expected loader-time behavior was to reject any reference that resolves to an existing local file/directory/symlink in the actor cwd, including a local `FS_DILOCO_HF_WIKITEXT_REPO` override. Actual behavior reached the mocked producer in all four cases: relative local model directory, relative model symlink, ambiguous relative dataset directory, and absolute local env override. Focused (which does not include the root-level loader file) passed `673`; full was `799 passed, 4 failed, 2 skipped`; zero errors. Artifact: `artifacts/20260810-074000_plan-complete-loader-local-red-attempt1-fail.json`; raw logs/JUnit: `logs/qsub_plan03_p6_g2_2514566/`; stdout: `fsdiloco_p6_g2.o2514566`.
- Confirmed cause: valid Hub spellings and local relative paths share the same string syntax; config-only prefix checks cannot know the actor cwd. Transformers/datasets resolve an existing path locally and revision kwargs do not freeze its bytes. The environment override is applied after config validation. All other tests passed, so the finding is isolated to the loader-time identity boundary.
- Next modification: add one shared modeling-layer guard immediately before producer import/call that expands `~`, resolves existence without following a missing target, and rejects any existing path/symlink with the descriptor-content-identity diagnostic. Apply it to configured model, resolved primary dataset (including env override), and fallback target; keep non-existing Hub IDs and exact revision kwargs unchanged. Rerun the same experiment.
