...
加上 `--submit --syncer-walltime HH:MM:SS --learner-walltime HH:MM:SS --log-root /absolute/log/root` 才会提交独立 PBS actor；两个 walltime 都必须显式给出且至少十分钟。static 模式为每个 learner 提交一条 scalar PBS job，不使用 job array。正式 9-node 配置是 `configs/full_protocol_static.yaml`，工作量固定为每轮 50 个 local optimizer steps、10 个 committed global steps。
