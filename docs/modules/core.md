...
`ConfigV4` 包含 shared config、schema version、`LeaderSection`、`MaintenanceSection` 和 `stop_after_direct_weight_tokens_applied`。`load_config_v4` 拒绝 old init/fragment/failure/HA/recovery/token key，`resolve_config_v4` 产生 fresh run identity。`migrate_v3_bytes_to_v4` 只做有限、可证明的 config 迁移。
