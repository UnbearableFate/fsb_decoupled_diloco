...
- `TinyCausalLM` 是 embedding 加无 bias linear head;`forward()` 返回全序列 logits,传 labels 时用 `logits[:, :-1]` 对 `labels[:, 1:]` 做均值交叉熵。
...
| `wikitext_batches()` | 先加载 train split,再 `dataset.shard(..., contiguous=True)`,物化 blocks,最后建立上述无限迭代器。即使 `data.streaming=true`,token/block 阶段仍物化全部块。 |
...
| `build_stream_batch_iterator()` | dynamic 入口;严格要求 `0 ≤ stream_id < stream_pool_size`,随后把 stream ID 映射为 learner index、固定 pool 映射为 num shards 并复用同一 synthetic/WikiText 实现。active 成员增减不改变 shard 或 shuffle seed;replacement 复用 stream 时数据映射保持不变,restart 语义由成员 stream epoch 另行记录。 |
