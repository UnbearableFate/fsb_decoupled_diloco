...
- **`append_csv_row(path, row, fieldnames=None)`** — 追加一行;文件不存在或为空时先写表头;未传 fieldnames 时使用当行键顺序，`extrasaction="ignore"` 忽略多余键。它依赖文件 close 刷新，没有显式 fsync、原子替换或进程间锁。`syncer_metrics.csv` 是单写者；`learner_metrics.csv` 与 `update_manifest.csv` 由所有 learner 共享追加，存在并发首表头/写入竞态，因此只能作 best-effort 遥测，权威生命周期仍看 SQLite + archive。
