...
def initialize_new_run(
    ...
    *,
    ...
    if expected["mode"] != "full":
        raise ValueError("HA schema bootstrap supports full mode only")
    ...
    try:
        ...
        conn.execute(
            ...
            """,
            (HA_SCHEMA_VERSION, PROTOCOL_VERSION, expected["mode"], created_at),
        )
        for key, value in (
        ...
        conn.execute(f"PRAGMA user_version={HA_SCHEMA_VERSION}")
        conn.commit()
...
def open_existing(
    ...
    *,
    ...
    try:
        ...
        if schema_values != (HA_SCHEMA_VERSION, PROTOCOL_VERSION, expected["mode"]):
        ...
        if state_row is None or json.loads(state_row[0]) != HA_SCHEMA_VERSION:
        ...
        if int(conn.execute("PRAGMA user_version").fetchone()[0]) != HA_SCHEMA_VERSION:
